...
// Order History for Both Ongoing order and History order
export default class OrderDetailsSecreen extends React.Component {
  ...
  handlePopupSubmit = async () => {
    ...
    if (
      ...
      fetch(
        `${apiUrl}/finish_order${this.state.order.orderid}`,
  ...
  componentDidMount = async () => {
    ...
    let isCalledFromNotification = navigation.getParam("isCalledFromNotification");
    console.log("isCalledFromNotification", isCalledFromNotification)
    ...
    this.setState({ orderIdNum: isCalledFromNotification ? order : order.order_id }); //.substring(3)
    ...
    // console.log("link", `http://app.xn--mgbt1ckekl.com/api/cu/v.3/app/orders/${isCalledFromNotification ? order : order.order_id}`,
    // )
    // console.log(
    //   "http://app.xn--mgbt1ckekl.com/api/cu/v.3/app/orders/" + order.order_id
    // );
    fetch(
      `${apiUrl}/orders/${isCalledFromNotification ? order : order.order_id}`,
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        // console.log("order detail response", responseJson);
        if (responseJson) {
          //.error === false)
          console.log("Order Response in order Details", responseJson.orders);
          this.setState({
          ...
          // console.log("checg", responseJson.orders.order_id);
        } else {
    ...
    if (this.state.orderDetail.is_support) {
      if (
        ...
        this.props.navigation.navigate("ComplaintForm", {
          ...
          orderId: this.state.orderDetail.support.orderid,
        });
      } else {
  ...
  submitRescheduleRequest = () => {
    ...
    if (this.state.time && this.state.date !== null) {
      let obj = {
        order_id: this.state.order.order_id,
        desire_date: this.state.date + " " + this.state.time, //servicedate
        // servicetimeid: this.state.timeInterval[index].id, //time send ...
      };
      // console.log("reschedule order", obj);
      fetch(
        `${apiUrl}/order_reschedule`,
        ...
        .then((responseJson) => {
          ...
          // console.log("errorRes", responseJson);
          // if (responseJson.error === false) {
          Toast.show({
            text:
              this.state.lan === "en"
  ...
  render() {
    // console.log("condition", this.state.orderDetail?.sp_price_status !== "cus_approved")
    // console.log("<><><><><>", this.state.orderDetail?.sp_price_status)
        ...
        >
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    {this.state.orderDetail &&
                      this.state.orderDetail?.desire_date_format}
                  </Text>
                  <Text
                    style={{
                      color: "#4a4b4c",
                  ...
                  >
                    ...
                    {this.state.orderDetail?.order_status == 1
                      ? this.state.lan == "en"
                        ? "Request Sent"
                        : "تم ارسال الطلب"
                      : this.state.orderDetail?.order_status == 2
                        ? this.state.lan == "en"
                          ? "Professional Assigned"
                          : "تم تعيين المهني"
                        : this.state.orderDetail?.order_status == 3
                          ? this.state.lan == "en"
                            ? "Cancelled"
                            : "تم الحذف"
                          : this.state.orderDetail?.order_status == 4
                            ? this.state.lan == "en"
                              ? "Job Started"
                              : "المهمة بدأت"
                            : this.state.orderDetail?.order_status == 5
                              ? this.state.lan == "en"
                                ? "Completed"
                                : "تم إنجاز المهمة"
                              : this.state.orderDetail?.order_status == 6 && this.state.orderDetail?.sp_id ? this.state.lan == "en"
                                ? "Professional Assigned"
                                : "تم تعيين المهني"
                                : ""
                    }
            ...
            {(!this.state.isHistory &&
              this.state.orderDetail &&
              this.state.orderDetail?.order_status < 4) ||
              this.state.orderDetail?.order_status === 6 ? (
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                ...
                {this.state.orderDetail?.sp_price_status !== "cus_approved" ? (
                ...
                >
                  {this.state.lan === "en" ? (
                    ...
                    >
                      ...
                      you'll get 300 points and they'll get
                    ...
                    >
                      ...
                      خدمة ستربح 😊 300 نقطة، وسيحصلون هم على
