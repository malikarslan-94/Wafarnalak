...
export default class ComplaintFormScreen extends React.Component {
  ...
  componentDidMount = () => {
    ...
    console.log(
      "3000",
      navigation.getParam("support"),
      navigation.getParam("orderId")
    );
  };
  getComplaintTicket = (orderid) => {
    fetch(
      `http://app.xn--mgbt1ckekl.com/api/cu/v.3/app/get_ticket/${orderid}`,
      // `https://demo.xn--mgbt1ckekl.com/api/cu/v.3/get_ticket/${orderid}`,
      ...
      .then((response) =>
        // console.log("pyt", response);
        response.json()
      )
      .then((responseJson) => {
        console.log("lp9", responseJson);
        this.setState({ support: responseJson });
        console.log(
          `https://demo.xn--mgbt1ckekl.com/app/get_ticket/${orderid}`
        );
      })
      .catch((error) => {
        console.log(error);
  ...
  submitComplaint = () => {
    if (this.state.complaintText !== "") {
      fetch(
        ...
        "http://app.xn--mgbt1ckekl.com/api/cu/v.3/app/add_ticket",
        {
          method: "POST",
          headers: {
            Accept: "application/json",
