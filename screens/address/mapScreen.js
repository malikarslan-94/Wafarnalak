...
export default class GoogleMapScreen extends React.Component {
  ...
  fetchAddress = (lotlng) => {
    fetch(
      ...
      .then((responseJson) => {
        let r = responseJson.results[0].address_components.filter(
          (name) => name.types[0] == "locality"
        );
        console.log("Mapi", responseJson.results[0]);
        console.log("iop");
        console.log("mai", r);
        this.setState({
        ...
        >
          <TouchableOpacity onPress={this.saveAddress}>
            ...
            >
              ...
              >
                <Text
                  style={{ color: "white", margin: 12, alignSelf: "center", fontFamily: this.state.lan == "en" ? "Montserrat_semiBold" : "montserrat_arabic_regular", fontSize: this.state.lan === "en" ? 18 : 11, }}
                >
                  {lanConfirm == "en" ? "Select Location" : "حدد العنوان"}
                </Text>
              </LinearGradient>
