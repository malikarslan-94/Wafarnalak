...
const HelpScreen = ({ navigation }) => {
  ...
  const openFaqs = () => {
    if (lan === "en") {
      WebBrowser.openBrowserAsync(help.faq.en_url);
    } else {
      WebBrowser.openBrowserAsync(help.faq.en_url);
      // WebBrowser.openBrowserAsync(help.faq.ar_url);
    }
  };
  ...
  useEffect(() => {
    ...
    fetch(
      `${apiUrl}/help`,
      // "http://app.xn--mgbt1ckekl.com/api/cu/v.3/app/help",
      ...
      .then((responseJson) => {
        console.log("helpRJ", responseJson);
        setHelp(responseJson)
        setLan(lan)
        setLoading(false)
        // setState({ help: responseJson, lan: lan, loading: false });
      })
      .catch((error) => {
        setLoading(false)
      });
  ...
  //   );
  // };
  const openPrivacyFaqs = () => {
    if (lan === "en") {
      WebBrowser.openBrowserAsync(help.privacy_faq.en_url);
    } else {
      WebBrowser.openBrowserAsync(help.privacy_faq.en_url);
      // WebBrowser.openBrowserAsync(help.privacy_faq.ar_url);
    }
  };
          ...
          <View style={{ marginTop: Dimensions.get("screen").height / 3.5 }}>
            <TouchableOpacity onPress={openFaqs}>
              ...
              >
                ...
                >
                  <Text style={{
                    color: "#4a4b4c", fontFamily: lan == "en" ? "" : "montserrat_arabic_regular", fontSize: lan === "en" ? 17 : 10,
                  }}>
                    {lan == "en" ? "FAQs" : "اسئلة مكرره"}
                  </Text>
                  <Ionicons
                    name={
                      lan == "en"
            ...
            <TouchableOpacity onPress={openPrivacyFaqs}>
              <View
                style={{
                  width: Dimensions.get("screen").width - 60,
                  backgroundColor: "white",
